...
(function (Slug) {
    var Main = /** @class */ (function () {
        ...
        Main.prototype.slug = function (value, sep) {
            ...
            value = value.toLowerCase().replace(new RegExp('[^' + Str.pregQuote(separator) + '\\d\\w\\s]', 'ug'), '');
            value = value.replace(new RegExp('[' + Str.pregQuote(separator) + '\\s]+', 'ug'), separator);
            return value.trim();
        };
        Main.ascii = function (value) {
            _.forEach(Str.charsArray(), function (val, key) {
                value = Str.replaceArray(value, val, key);
            });
            return value.replace(/[^\x20-\x7E]/u, '');
        };
        Main.pregQuote = function (str, delimiter) {
            return (str + '').replace(new RegExp('[.\\\\+*?\\[\\^\\]$(){}=!<>|:\\' + (delimiter || '') + '-]', 'g'), '\\$&');
        };
        Main.replaceArray = function (string, find, replace) {
